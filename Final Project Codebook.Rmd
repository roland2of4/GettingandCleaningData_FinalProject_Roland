...
From the Coursera project introduction, we know the following about the data:
...
Originally the data is spread out in several different files, which have to be combined in a certain way to create a single dataset for the project. The files are as follows (this is from the dataset's readme.txt):
...
- 'activity_labels.txt': Links the class labels with their activity name. There are 6 potential types of activity.
...
3) Details on the Features (variables)
The original list of 410 features (variables) is in features.txt
There is a file called features_info text which provides the following background:
...
4) Restructuring the Data Set
Here is the process followed to transform the original multiple files into a single data set and then into a second tidy dataset. 
